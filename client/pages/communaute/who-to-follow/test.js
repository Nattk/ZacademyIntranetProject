...
import FormulaireWho2Follow from '../../admin/Who-to-Follow/formulaire'
        ...
        <article id="who-to-follow" className="col-md-12 col-sm-12 col-xs-12 section-card" >
          ...
          <section className="col-md-12 col-sm-12 col-xs-12 section-article" >
            {this.state.fakeData.map((user, id) => (
              ...
              <ShowCard
                ...
                update={() => this.setState({
                  showModal: true, formulaire: true, formulaireUpdate: true, descriptionDelete: false, formulaireTitleAdd: false, id: user.id, firstName: user.firstName, lastName: user.lastName, fonction: user.fonction, description: user.description, linkLinkedin: user.linkLinkedin, linkGithub: user.linkGithub, linkTwitter: user.linkTwitter, image: this.state.img
                })}
              />
            ))}
            <Modal show={this.state.showModal} onClose={this.handleClose} titleModal={this.state.formulaireTitleAdd ? "Ajout d'un contact" : '' || this.state.formulaireUpdate ? 'Modification du contact' : ''}>
              {this.state.formulaire ? <FormulaireWho2Follow buttonName={this.state.formulaireUpdate ? 'Mettre à jour' : 'Ajouter'}
              /> : false}
